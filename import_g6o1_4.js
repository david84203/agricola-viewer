...
const sourceTextPath = process.env.G6_SOURCE_TEXT || 'C:\\Users\\User\\Downloads\\G6o1-o4.txt';
const sourceImageDir = process.env.G6_SOURCE_IMAGE_DIR || 'D:\\農家樂卡牌\\2016新版\\中文化';
const writeMode = process.argv.includes('--write');

const sectionNamePattern = /^G6o\d+$/;
...
if (sections.length !== 4) {
  throw new Error(`Expected 4 image sections, got ${sections.length}`);
}

const { cards: importedCards, counts } = parseSections(sections);
for (const { section, count } of counts) {
  if (count !== 9) {
    throw new Error(`${section} has ${count} cards; expected 9 cards per image.`);
  }
}
